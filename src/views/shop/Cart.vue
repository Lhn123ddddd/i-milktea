<template>
  ...
  <div class="cart">
    ...
    <div class="check">
      ...
      <div class="check__btn" @click="handleGoPayClick()" :class="{changecolor:changecolor}">去结算</div>
    </div>
  </div>
...
const changeColor = (total)=>{
  ...
  return {
    ...
    cleanCartProducts,setCartItemsChecked,changeColor
    }
}
 //结算商品页面跳转
  const usePayEffect = (price)=>{
    // const price= computed()
...
export default {
  ...
  setup() {
    ...
    const { 
      total, price, productList, setCartItemsChecked,allChecked,changeColor,
      changeCartItemInfo, changeCartItemChecked ,cleanCartProducts} = useCartEffect(shopId)
    const { handleGoPayClick } = usePayEffect(price);
    const {changecolor}=changeColor(total)
    ...
    return { 
      ...
      handleCartShowChange,handleGoPayClick,changecolor
      }
  }
}
